...
### Core Components
1.  **`verify_replicator.py`**: A logical audit script for the 148.5 kHz resonance pulse. It verifies the frequency required for the Lorentz-Trigger interaction with the bifilar-wound geometry.
2.  **`hex_coords.py`**: The physical coordinate map for the 7-magnet N52 Neodymium array. This geometry is designed to maximize the Magnetic Vector Potential ($A$) while canceling the magnetic flux ($B=0$).

## 🛠️ Global Bill of Materials (BOM)
To replicate the physical effects described in the simulations, the following hardware is required:
